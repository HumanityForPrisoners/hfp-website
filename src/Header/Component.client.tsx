...
export const HeaderClient: React.FC<HeaderClientProps> = ({ data }) => {
  ...
  return (
    ...
    <header
      className={clsx('z-20 w-full relative top-0 transition-colors duration-1000 bg-primary')}
      {...(theme ? { 'data-theme': theme } : {})}
    >
      <div className="py-2 grid grid-cols-12-90 gap-30 mx-auto relative w-full bg-secondary px-48">
        <nav className={clsx(socials.length > 0 ? 'show' : 'hidden', 'contents')}>
          ...
          <div className="col-start-7 col-span-6 flex gap-2 items-center justify-end">
            {contacts.map(({ emailSelect, phoneSelect, type }, i) => {
              ...
              const contactLink =
                type === 'phone' ? formatPhoneNumber(phoneSelect?.toString()) : emailSelect
              return (
