            ...
            return (
              <div key={i} className="group hover:cursor-pointer">
                <div className="flex gap-1 justify-center items-center">
                  <p className="text-popover-foreground ">{multipleLinks?.label}</p>
                  ...
                  {multipleLinks?.links?.map((subLink, j) => {
                    return (
                      <CMSLink
                        key={j}
                        appearance="link"
                        className="text-popover-foreground w-36 text-center rounded-none p-4 bg-primary border border-foreground hover:bg-secondary hover:text-primary transition-colors first:rounded-t-lg last:rounded-b-lg"
                        {...subLink.link}
                      />
                    )
                  })}
